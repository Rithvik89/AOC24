...
func main() {
	...
	for {
		...
		if calc(level_int_) == 1 {
			ans++
			continue
		}

		ans += tolerate(level_int_)
...
func tolerate(level_int_ []int) int {

	for idx, _ := range level_int_ {
		// modify level_int_
		var li []int
		if idx == 0 {
			li = level_int_[1:]
		} else if idx == len(level_int_)-1 {
			li = level_int_[0 : len(level_int_)-1]
		} else {
			prefix := level_int_[:idx]
			suffix := level_int_[idx+1:]
